...
namespace SyncDbContext.Helpers
{
    public static partial class EFExtensions
    {
        ...
        public static async Task<(int, int)> HandleEntities<TEntity>(this DbContext context, List<TEntity> addedOrUpdated, List<TEntity> deleted, UpsertModel<TEntity> model) where TEntity : class
        {
            ...
            {
                deletedCount = await Delete(context, deleted, model);
            }
            if (addedOrUpdated?.Any() ?? false)
            {
                addedOrUpdatedCount = await Upsert(context, addedOrUpdated, model);
            }

            return (addedOrUpdatedCount, deletedCount);
        }
    }
    ...
    public abstract class EntityOp<TEntity, TRet> where TEntity : class
    {
        ...
        protected readonly bool _hasOnlyPrimaryKeys;
        ...
        public EntityOp(DbContext context, IEnumerable<TEntity> entityList, UpsertModel<TEntity> model) 
        {
            ...
            _hasOnlyPrimaryKeys = model.EntityHasOnlyKeys;
        }
    ...
    public class UpsertOp<TEntity> : EntityOp<TEntity, int> where TEntity : class
    {
        ...
        public async override Task<int> Execute()
        {
            ...
            //only update for tables that have props other than PKs
            if (!_hasOnlyPrimaryKeys)
            {
    ...
    public class DeleteOp<TEntity> : EntityOp<TEntity, int> where TEntity : class
    {
        public DeleteOp(DbContext context, IEnumerable<TEntity> entityList, UpsertModel<TEntity> model) : base(context, entityList, model)
        {
        }
